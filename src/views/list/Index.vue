  ...
  <div class="list-wrapper">
    <yd-accordion style="position: relative;padding-bottom: 50px;" v-if="devGroupList.length">
      <yd-accordion-item open="open" v-for="(item,index) in devGroupList" :key="index" v-press="press">
        ...
        <div slot="txt" style="color:#20aaf8;margin-left:5px; position: absolute; right: 90px;"
             v-show="groupFlag && item.groupName !== '默认组'" @click="OpenGroup(item)">编辑
        </div>
        <div slot="txt" style="color:#20aaf8;margin-left:5px; position: absolute; right: 60px;"
             v-show="groupFlag && item.groupName !== '默认组'" @click="deleteGroup(item.groupId)">删除
        </div>
        <div slot="txt" style="color:#20aaf8;margin-left:5px; position: absolute; right: 30px;"
             v-show="groupFlag && item.groupName !== '默认组'" @click="groupFlag = false">取消
        </div>
        <div style="padding:15px">
          <div class="list-item" v-swipeleft="swipeleft" v-swiperight="swiperight"
               @click="intoIndex(child.deviceId,child.deviceName,child.icon,child.deviceTypeName)"
               v-for="(child,cindex) in item.deviceItemPos" :key="cindex">
            <div class="item-left">
              <div class="icon" :class="{ active : containIds(child.deviceId) }" v-if="loopValue === true"
                   @click.stop="selectDev(child.deviceId)"></div>
              <div class="img">
                <div class="p-img">
                  <img :src="child.icon">
                ...
                <div class="img-text">
                  <p><span>{{ child.deviceName }}</span><i
                    class="addr"></i><span>{{child.location.split(',')[1]}}</span></p>
            ...
            <div class="item-right">
              ...
              <span class="delete" v-if="loopValue === true" @click.stop="deleteDev(child)"
                    style="color: #a0a0a0;">删除</span>
              <p>PM2.5 </p>
              <p>{{child.pm}}ug/m³</p>
            </div>
          </div>
    ...
    <div class="create-dialog dialog" v-if="editDevFlag">
      <div class="confirm">
        <div class="confim-top">
          <textarea class="name" style="min-height:50px;" placeholder="输入新设备的名称"
                    v-model="devInfo.deviceName"></textarea>
        </div>
        <div class="confim-bottom">
        ...
        <div class="confim-top" style="text-align: left; color: #fff; padding: 0 10px 10px; ">
          ...
          <div>
            <yd-radio-group v-model="selectId" color="#00ff00">
              <yd-radio :val="item.groupId" v-if="currentGroupName != item.groupName"
                        style="margin-top:10px;width:100%;color: #fff " v-for="(item,index) in devGroupList"
