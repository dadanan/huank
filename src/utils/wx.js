...
export function sign () {
  ...
  let url = myUrl.getSign + '?url=' + targetUrl
  ...
  Vue.prototype.$http.get(url).then(res => {
    if (res.code === 200) {
      wx.config({
        ...
        jsApiList: ['onMenuShareAppMessage', 'onMenuShareTimeline', 'hideMenuItems', 'openLocation', 'getLocation', 'showMenuItems', 'scanQRCode'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
      })
    }
  }).catch(error => {
